...
class DanProductivityTracker(app_callback_class):
    def __init__(self):
        ...
        self.current_status = "No one looking at Window display"
        self.no_persons_percentage = 100.0
...
def dan_productivity_callback(pad, info, user_data):
    ...
    if person_detected:
        ...
        user_data.current_status = "Person(s) detected looking at display"
    else:
        user_data.current_status = "No one detected looking at display"
    
    # Calculate percentage
    if user_data.total_frames > 0:
        user_data.persons_percentage = 100.0 * user_data.frames_with_person / user_data.total_frames
    ...
    if user_data.use_frame and frame is not None:
        ...
        # Percentage
        cv2.putText(frame, f"Persons Looking at Display: {user_data.persons_percentage:.1f}%", 
    ...
    # Console output
    print(f"\rFrames: {user_data.total_frames} | Status: {user_data.current_status} | Persons: {user_data.persons_percentage:.1f}%", end="")
    
    return Gst.PadProbeReturn.OK

if __name__ == "__main__":
    # Create tracker and app
    print("[INFO] Display Window Monitor...")
