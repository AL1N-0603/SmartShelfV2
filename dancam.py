...
# Set up page
st.set_page_config(page_title="DAN PRODUCTIVITY MONITOR", layout="wide")
st.title("DAN PRODUCTIVITY MONITOR")

# Find detection script
def get_detection_script_path():
    possible_paths = [
        "./detection.py",
        "../detection.py",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "detection.py"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "detection.py")
    ]
...
# Instructions
st.markdown("""
## DAN PRODUCTIVITY MONITOR

This application tracks Dan's productivity based on person detection.
- **Dan's working** - When a person is detected
- **Dan's goofing off** - When no person is detected

Click 'Start Monitoring' to begin.
""")

# Create a simple modification script that doesn't require string replacements
if st.button("Start Monitoring"):
    detection_script = get_detection_script_path()
    
    if not detection_script:
        st.error("Could not find detection.py script.")
    else:
        st.success(f"Found detection.py at: {detection_script}")
        
        # Create a standalone monitoring script
        monitor_script = """import gi
...
"""
        ...
        monitor_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dan_monitor.py")
        with open(monitor_path, 'w') as f:
            f.write(monitor_script)
        
        st.success("Created Dan's productivity monitoring script")
        
        # Run the standalone monitor script
        cmd = [sys.executable, monitor_path, "--input", "rpi", "--use-frame"]
        
        st.info("Launching monitor... A video window should appear")
        ...
        try:
            ...
            st.info("You can close this browser tab when done")
else:
